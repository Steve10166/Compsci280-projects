...
# ----------------- Alignment -----------------
def align(ref, mov):
    s = [0.125, 0.25, 0.5, 1.0] #4 layed pyramid image.
...
def trim(rgb):
    ...
    ig, isat = g[int(0.10*h):int(0.90*h), int(0.10*w):int(0.90*w)], sat[int(0.10*h):int(0.90*h), int(0.10*w):int(0.90*w)]  #samples middle image 

    dark   = np.quantile(ig, 0.05)
    bright = np.quantile(ig, 0.98)               
    sref   = np.median(isat) + 0.28  # set threholds for below %brightness to be considered dark, above %brightness to be considered white, above %average saturation to be considered colored edge.

    rim = (g < dark) | (g > bright) | (sat > sref)

    def scan_rows(start, step, thr, win=22):  #continuously scan for windows in a row and 
        i, keep = start, False
        while 0 <= i < h:
...
with open("results/offsets.csv", "w", newline="") as f:
    writer = csv.writer(f)
    writer.writerow(["filename", "gx", "gy", "rx", "ry"])
    ...
    for p in paths:
        ...
        writer.writerow([os.path.basename(p), sg[1], sg[0], sr[1], sr[0]])
